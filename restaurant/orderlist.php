<?php
  ...
  mysqli_select_db($conn, "foody") or die(mysqli_connect_error());
    ...
    $stqlise ="SELECT * FROM orderlist INNER JOIN menu ON orderlist.menuid = menu.menuid";
...
<nav class="sidebar">
  ...
  <b><a href="orderlist.php">Order list</a></b><br>
  <b><a href="menu.php">Menu for customer</a></b><br>
      ...
      <br>
        ...
        <?php
                ...
                while($row = mysqli_fetch_array($sp)){ 
                    $date = strtotime($row['orderlistdate']);
                    ?> 
                    ...
                    <input type="hidden"  name="id[]" value="<?=$row['orderlistid']?>">
                      ...
                      <td>
                        <select name="state[]" align="center">
                          <option><?php echo $row['orderstate']?></option>
    ...
    <?php 
        $dateorderlist ="SELECT MONTHNAME(orderlistdate) AS mth, sum(orderquantity) as countquan  
        FROM orderlist
        GROUP BY MONTHNAME(orderlistdate)
        ORDER BY MONTHNAME(orderlistdate)";
